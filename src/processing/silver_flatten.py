...
def flatten_comment_files(spark) -> "pyspark.sql.DataFrame":
    ...
    """
    # where ur gonna store every single comment now 
    rows = []
    ...
    for filepath in glob.glob(str(BRONZE_DIR / "comments_*.json")):
        ...
        # so just parsing it from here 
        video_id = Path(filepath).stem.split("_")[1]

        # python will turn json file nested Python dicts and lists
        ...
        for page in pages:
            for item in page.get("items", []):
                top = item["snippet"]["topLevelComment"]["snippet"] # go 
                rows.append({
                    "comment_id": item["id"],
    ...
    # turn rows into tables u can filter, transform and modify it now 
    df = spark.createDataFrame(rows)
    df = df.dropDuplicates(["comment_id"]) # if its comming from the same person + same text, drop dups 
    return df
